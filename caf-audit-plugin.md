...
client side Java class named `AuditLog`. This auto-generated class comprises methods for sending audit event messages to 
the Apache Kafka messaging service.  
...
<plugin>
	...
	<artifactId>caf-audit-plugin</artifactId>
...
Calls to methods `auditViewDocument` and `auditPolicyApplied` would then be made to send document and policy event messages 
respectively to Kafka.
